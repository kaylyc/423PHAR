...
`penguins` contains `r nrow(penguins)` observations collected and made available by Dr. Kristen Gorman and the Palmer Station, Antarctica LTER[^1].

[^1]: Horst AM, Hill AP, Gorman KB (2020).
...
2.  What does the `bill_depth_mm` variable in the `penguins` data frame describe?
    ...
    bill_depth_mm is a number denoting bill length (millimeters)
...
4.  What happens if you make a scatterplot of `species` vs. `bill_depth_mm`?
    ...
    A box plot might be a better choice.

    ```{r}
    ggplot(data = penguins,mapping = aes(x=species, y=bill_depth_mm, colour=species))+geom_point()
    ```

5.  Why does the following give an error and how would you fix it?

    There is no mapping specification.
    Add mapping via aes function.
...
6.  What does the `na.rm` argument do in `geom_point()`?
    ...
    The na.rm can remove missing values.
...
9.  Run this code in your head and predict what the output will look like.
    ...
    Scatterplot will be generated examining the relationship between flipper length and body mass.
    There will be different coloured points based on the number of islands in the dataset.
    Smooth lines will be generated to identify patterns.
...
10. Will these two graphs look different?
    ...
    Yes because the variables mapped are the same.
    The function generates ggplot and also adding points and smoothed lines.
...
1.  Make a bar plot of `species` of `penguins`, where you assign `species` to the `y` aesthetic.
    ...
    This bar plot displays the bars horizontally.
...
2.  How are the following two plots different?
    ...
    Fill is more useful to change the colours of the bars
...
3.  What does the `bins` argument in `geom_histogram()` do?
    ...
    The bins argument indicates the number of bars in the histogram.
...
4.  Make a histogram of the `carat` variable in the `diamonds` dataset that is available when you load the tidyverse package.
    ...
    Smaller binwidth create the most interesting patterns (e.g., binwidth=0.2)
...
The first argument of `facet_wrap()` is a formula[^2], which you create with `~` followed by a variable name.
The variable that you pass to `facet_wrap()` should be categorical.

[^2]: Here "formula" is the name of the thing created by `~`, not a synonym for "equation".
...
1.  The `mpg` data frame that is bundled with the ggplot2 package contains `r nrow(mpg)` observations collected by the US Environmental Protection Agency on `r mpg |> distinct(model) |> nrow()` car models.
    ...
    There are 6 categorical variables (manufacturer, model, trans, drv, fl, class).
    The other 5 variables are numerical.
...
2.  Make a scatterplot of `hwy` vs. `displ` using the `mpg` data frame.
    ...
    Colour can give a gradient but visually can be hard to distinguish.
    ...
    ggplot(mpg, aes(x = hwy, y = displ)
      ...
      geom_point(aes(colour = displ, size = cyl, shape = hwy))
    ```
